...
def test_clock_plus(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
...
def test_clock_plus_linux(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_clock_plus_macos(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
def test_clock_plus_host(app: QemuApp, dut: QemuDut) -> None:
